...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        classifier_path="D:/FYP/Project/recognizer.h5"
        h = 32
        w = 32
        traffic_signs = { 0:'Speed limit (20km/h)',1:'Speed limit (30km/h)', 
                    ...
                    36:'No passing',37:'Go straight or left', 
        ...
        for i, det in enumerate(pred):  # per image
            ...
            p = Path(p)  # to Path
            save_path = str(save_dir / "output.jpg")  # im.jpg
            save_path1 = str(save_dir / "yolo.jpg")
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        annotator1.box_label(xyxy, names[c], color=colors(c, True))

                        crop = save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=False, save=False)
                        image = cv2.resize(crop, (h, w))
                        image = np.expand_dims(image,axis=0)
                        res = classifier_model.predict(image)[0]
                        idx = np.argmax(res)
                        conf = res[idx]
                        name = traffic_signs[idx]
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path1[i] != save_path1:  # new video
                        vid_path1[i] = save_path1
                        ...
                        save_path1 = str(Path(save_path1).with_suffix('.mp4'))  # force *.mp4 suffix on results videos
                        vid_writer1[i] = cv2.VideoWriter(save_path1, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
                    vid_writer1[i].write(im1)
